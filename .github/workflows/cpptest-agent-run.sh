...
# Register C/C++test MCP server - assuming C/C++test is installed in '/opt/parasoft/cpptest' location, adjust as needed
/home/syoung/.local/bin/copilot mcp get cpptest-std-mcp >/dev/null 2>&1 || copilot mcp add cpptest-std-mcp -- /opt/parasoft/cpptest/integration/mcp/cpptestmcp

# Execute the prompt with Copilot - be sure to adjust sandbox permissions as needed for your prompt
/home/syoung/.local/bin/copilot --allow-all --no-ask-user -s -p "$(cat "$(dirname "$0")/cpptest-agent-prompt.md")"